...
def load_data():
    ...
    try:
        conn = psycopg2.connect(
            ...
            password=os.environ.get("DB_PASSWORD", "subhashini"),
...
def create_slicing_charts(df):
    ...
    yearly_avg = (
        ...
        .mean()
        .reset_index()
        .rename(columns={'date': 'Year', 'total_nonfarm': 'Average Employment'})
    )
    yearly_avg.columns = ['Year', 'Average Employment']
    st.dataframe(yearly_avg.style.format({'Average Employment': '{:,.0f}'}), use_container_width=True)
    fig_yearly = px.bar(
        yearly_avg, x='Year', y='Average Employment',
        title=f"Average Payroll Employment by Year ({year_range[0]}–{year_range[1]})",
    ...
    selected_presidents = [
        ...
        if not (year_range[1] < p[0] or year_range[0] > p[1])
    ]
    presidents_str = ", ".join(selected_presidents) if selected_presidents else "No president data for this range."
    st.text_input(
        "Presidents of the USA during selected interval:",
        value=presidents_str,
...
def create_dicing_charts(df):
    ...
    # Q4: Which month shows the highest payroll growth in each year?
    if quarter == 'Q4':
        q4_growth = df_quarter_interval.copy()
        # For each year, find the month with the highest MoM % growth
        q4_winners = (
            q4_growth.loc[q4_growth.groupby('year')['pct_change_mom'].idxmax()]
            .sort_values('year')[['year', 'month', 'pct_change_mom']]
            .rename(columns={'month': 'Top Month', 'pct_change_mom': 'Max MoM % Growth'})
        )
        st.subheader("Q4: Month with Highest Payroll Growth Each Year")
        st.dataframe(q4_winners.style.format({'Max MoM % Growth': '{:.2f}'}), use_container_width=True)

        # Summary: Which month is most often the winner?
        winner_counts = q4_winners['Top Month'].value_counts().reset_index()
        winner_counts.columns = ['Month', 'Number of Years as Q4 Winner']
        st.write("Summary: Which Q4 month most often has the highest payroll growth?")
        st.dataframe(winner_counts)

    # Table: Quarterly totals and growth rates
    st.write("**Quarterly Total Employment and QoQ Growth Rate**")
    st.dataframe(
        df_quarter_filtered[['year', 'quarter', 'total_nonfarm', 'qoq_growth']]
        .rename(columns={
            'year': 'Year',
            'quarter': 'Quarter',
            'total_nonfarm': 'Total Employment',
            'qoq_growth': 'QoQ Growth (%)'
        })
        .round({'Total Employment': 0, 'QoQ Growth (%)': 2}),
...
def create_roll_up_charts(df):
    ...
    # Table: Annual totals and growth rates
    st.write("**Annual Total Employment and YoY Growth Rate**")
    st.dataframe(
        df_yearly_interval[['year', 'total_nonfarm', 'yoy_growth']]
        .rename(columns={
            'year': 'Year',
            'total_nonfarm': 'Total Employment',
            'yoy_growth': 'YoY Growth (%)'
        })
        .round({'Total Employment': 0, 'YoY Growth (%)': 2}),
        use_container_width=True
    )
...
def create_drill_down_charts(df):
    ...
    highest_gain_df['month_num'] = highest_gain_df['date'].dt.month
    highest_gain_df['monthly_change'] = highest_gain_df['total_nonfarm'].diff()
    ...
    if view_option == "Month":
        ...
        # Show table of monthly changes
        st.write("**Monthly Employment Changes**")
        st.dataframe(
            highest_gain_df[['month_num', 'month', 'total_nonfarm', 'monthly_change']]
            .rename(columns={
                'month_num': 'Month #',
                'month': 'Month',
                'total_nonfarm': 'Total Employment',
                'monthly_change': 'Change from Previous Month'
            })
            .round({'Total Employment': 0, 'Change from Previous Month': 2}),
            use_container_width=True
        )
    else:
